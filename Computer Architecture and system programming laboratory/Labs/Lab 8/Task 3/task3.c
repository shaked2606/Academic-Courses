...
void printSymbols(state *s){
	...
	Elf32_Ehdr *header;				// this will point to the header structure
	Elf32_Shdr *section_header;			// this will point to the section header
	Elf32_Sym *symtab = NULL;			// this will point to the symtab table
	Elf32_Sym *dynsym = NULL;			// this will point to the dynsym table
	...
	// when debug_mode is on
	if(s->debug_mode == 1){
		printf("DEBUG INFO in printSymbols function:\n");
		printf("file_name: %s\n", s->file_name);
		printf("number symbols in symtab: %d\n",section_header[index_of_symtab_table].sh_size/sizeof(Elf32_Sym)); 	
		printf("number symbols in dynsym: %d\n",section_header[index_of_dynsym_table].sh_size/sizeof(Elf32_Sym)); 	
...
void relocationTables(state *s){
	...
	char* start_dynstr_string = NULL;	// beginning of string of name of symbols

	header = (Elf32_Ehdr*) map_start;	// header will point to the beginning of the mapped file
	section_header = (Elf32_Shdr*)(map_start + header->e_shoff);	// will point to the beginning of the section header
	Elf32_Shdr *sh_strtab = &section_header[header->e_shstrndx]; 	// point to entry of name section table
	start_name_section  = map_start + sh_strtab->sh_offset;			// point to the beginning of the string of name sections
	...
	for(i=0; i<header->e_shnum;i++){
		if(section_header[i].sh_type == SHT_REL){	// check if the entry is relocation table
			relocation_table = (Elf32_Rel *)(map_start + section_header[i].sh_offset);
			number_of_enteries = section_header[i].sh_size/sizeof(Elf32_Rel);
			printf("Relocation section '%s' at offset %#x contains %d entries:\n",start_name_section+section_header[i].sh_name, section_header[i].sh_offset, number_of_enteries);
			printf("Offset\t  Info\t\tType\tSym.Value\tSym.Name\n");

			// loop over the relocation table enteries
			for(j=0;j<number_of_enteries;j++){
				printf("%08x %08x\t%d\t%08x\t%s\n",relocation_table[j].r_offset,relocation_table[j].r_info, ELF32_R_TYPE(relocation_table[j].r_info), dynsym[ELF32_R_SYM(relocation_table[j].r_info)].st_value, start_dynstr_string + dynsym[ELF32_R_SYM(relocation_table[j].r_info)].st_name);
			}
		}
	}
	...
	// when debug_mode is on
	if(s->debug_mode == 1){
		printf("DEBUG INFO in relocationTable function:\n");
		printf("file_name: %s\n", s->file_name);
		printf("number of enteries in relocation table: %d\n",number_of_enteries);	
	}
}
...
int main(int argc, char **argv) {
	...
	int action = 0;
	char option[128];
	struct fun_desc menu[] = {{"Toggle Debug Mode",toggleDebugMode}, {"Examine ELF File",examineElfFile},{"Print Section Names",printSectionNames},{"Print Symbols",printSymbols},{"Relocation Table", relocationTables}, {"Quit",quit}, {"NULL",NULL}};
