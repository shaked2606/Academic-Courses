...
let my_ops = 
    ...
    mov r8, rsp ; backing up original stack pointer (after pushing rbp)
    mov rdi, [rbp + WORD_SIZE * 3] ; number of params (including magic)
    mov rsi, [rbp + WORD_SIZE * (rdi + 2)] ; variadic list

    mov r14, rbp
    lea r14, [r14 + (rdi + 4) * WORD_SIZE]      ; points to the start of the prev frame

    mov rbx, [rbp + WORD_SIZE * 4] ; procedure
    mov rcx, 0 ; init counter

    ; counting variadic list length
    Compute_Length_List:
    cmp rsi, SOB_NIL_ADDRESS
    je Finish_Computing_Length_List
    CDR rsi, rsi
    inc rcx
    jmp Compute_Length_List

    ; r8 = original stack pointer (after pushing rbp)
    ; rdi = number of params (including magic)
    ; rbx = procedure
    ; rcx = length of variadic list


    Finish_Computing_Length_List:
    mov rsi, [rsp + WORD_SIZE * (rdi + 2)] ; taking variadic list
    push qword SOB_NIL_ADDRESS ; pushing magic first 
    mov r9, rcx ; r9 = length of variadic list
    shl r9, 3    ; r9 = bytes space needed in stack for variadic list
    sub rsp, r9 ; reserve place for variadic list

    ; storing list elements in the stack
    mov r9, 0 
    Pushing_List_to_Stack:
    cmp rsi, SOB_NIL_ADDRESS
    je End_Of_Pushing_List_to_Stack
    CAR r10, rsi
    mov qword [rsp + WORD_SIZE * r9], r10
    CDR rsi, rsi
    inc r9
    jmp Pushing_List_to_Stack


    End_Of_Pushing_List_to_Stack:
    mov r9, rdi   ; taking number of params (including magic)
    sub r9, 3     ; sub magic, variadic list, and proc params
    shl r9, 3     ; r9 = bytes needed for params in stack (excluding proc, magic, and variadic list)
    sub rsp, r9   ; reserve place for params (excluding proc, magic, and variadic list)
  
    shr r9, 3     ; we use it as stop condition

    ; storing arguments outside the list in the stack
    mov r10, 0
    Pushing_To_Stack:
    cmp r10, r9
    je Calling_The_Proc
    mov r12, [r8 + WORD_SIZE * (r10 + 5)]
    lea r11, [rsp + WORD_SIZE * r10]
    mov qword [r11], r12
    inc r10
    jmp Pushing_To_Stack

    ; organize the stack with n, env, ret
    Calling_The_Proc: 
    mov r9, rcx                     ; length of variadic list
    add r9, r10                     ; number of params outside variadic list
    inc r9                          ; add magic
    push qword r9                   ; push new n (number of params including variadic list)

    mov r10, 0
    CLOSURE_ENV r10, rbx
    push qword r10                          ; env of proc

    mov r11, qword [rbp + WORD_SIZE * 1]    ; ret address of apply
    push qword r11                          ; push ret address of apply
    mov rdx, qword [rbp]                    ; save old_rbp

    ; set up the current frame with the new frame
    ; Init counter loop
    mov rcx, qword rbp   
    
    mov r9, [rbp + 3 * WORD_SIZE]

    ; shifting frame that was built to the old_frame
    mov r8, 0
    ; Start of set up frame loop
    Apply_Set_Up_Frame:
    cmp rcx, rsp                                    ; check if we got to the top of the stack
    jle Apply_Set_Up_Frame_Exit                     ; end of loop
    sub rcx, WORD_SIZE                              ; sub rcx by WORD_SIZE
    inc r8
    mov r13, qword [rcx]                            ; set rbx to be the [rcx] content
    mov qword [rcx + (r9 + 4) * WORD_SIZE], r13     ; do the set up
    jmp Apply_Set_Up_Frame                          ; continue loop 

    ; mov rbp to old_rbp and fix rsp
    Apply_Set_Up_Frame_Exit:
    lea r8, [r8 * WORD_SIZE]
    mov rbp, rdx 
    mov rsp, r14
    sub rsp, r8
    ...
    mov rax, SOB_VOID_ADDRESS" in
     ...
     (String.concat "\n\n" [(make_unary "car" car); (make_unary "cdr" cdr); (make_binary "cons" cons); (make_binary "set_car" set_car); (make_binary "set_cdr" set_cdr); apply])
