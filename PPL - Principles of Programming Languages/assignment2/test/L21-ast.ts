...
//L21
export interface ForExp {tag: "ForExp"; var: VarDecl; start: NumExp;
end: NumExp; body: CExp; };
...
// L21
export const makeForExp = (v: VarDecl, start: NumExp,
    end: NumExp, body: CExp): ForExp =>
...
// <ForExp> -> (for <VarDecl> <NumExp> <NumExp> <CExp>)
const parseForExp = (variable: Sexp, body:Sexp[]): Result<ForExp> =>
    ...
    bind(mapResult(parseL21CExp, body),
         (cexps: CExp[]) => 
         isNumExp(cexps[0]) && isNumExp(cexps[1]) ?
         makeOk(makeForExp(makeVarDecl(variable), cexps[0], cexps[1], cexps[2])) :
