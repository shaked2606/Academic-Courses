...
describe('L4 Normal Eval', () => {
    ...
    // Task 3.3
    // my tests: 
    it('my test1 - evaluates programs which would cause error in applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (define a (lambda (x) (/ 1 x)))
                (define b (lambda (x) 100))
                (b (a 0)))`), evalNormalProgram)).to.deep.equal(makeOk(100));
    });

    it('my test2 - evaluates programs which would cause semantic error in applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (define b (lambda (x) (+ b #t)))
                (define c (lambda (x) 7))
                (c (b 0)))`), evalNormalProgram)).to.deep.equal(makeOk(7));
    });

    it('my test3 - evaluates programs which would cause semantic error in applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (define b (lambda (x) (+ #t)))
                (define c (lambda (x) 10))
                (c (b 0)))`), evalNormalProgram)).to.deep.equal(makeOk(10));
    });
});
