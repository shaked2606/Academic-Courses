...
(define fold-left 
	(let ((null? null?) (car car) (cdr cdr))
		(letrec ((foldl (lambda (func acc lst)
				 (if (null? lst) 
				 	acc
...
(define fold-right
	(let ((null? null?) (car car) (cdr cdr))
		(letrec ((foldr (lambda (func acc lst)
				 (if (null? lst) 
				 	acc
...
(define cons*
	(let ((foldr fold-right) (null? null?) (car car) (cdr cdr) (cons cons))
	(lambda variadic
		(letrec ((build (lambda (variadic)
								(if (null? (cdr variadic))
								(car variadic)
								(build (cdr variadic)))))
				(rest (lambda (variadic)
				 				(if (null? (cdr variadic))
								 '()
								 (cons (car variadic)
								 (rest (cdr variadic)))))))
		(foldr cons (build variadic) (rest variadic))))))
