...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeEmptyTupleTExp, TupleTExp, makeNonEmptyTupleTExp, NonEmptyTupleTExp, isAtomicTExp, isProcTExp, isEmptyTupleTExp, isNonEmptyTupleTExp, isCompoundTExp } from './TExp';
...
//         | (values <cexp>*)                / ValuesExp(params: CExp[])
//         | (let-values (<let-values-binding>*) <cexp>+) /LetValuesTexp(bindings:LetValuesBinding[], body: CExp[])



// <binding>  ::= ( <var> <cexp> )            / Binding(var:VarDecl, val:Cexp)
// <let-values-binding> ::= (<var>* <cexp>)   / LetValuesBindings(var:VarDecl[], val:CExp)
...
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isValuesExp(e) ? e.params :
    isLetValuesExp(e) ? [...map((b) => b.val, e.bindings), ...e.body] :
...
/**************************************** Q2.1  ******************************************************/
export interface ValuesExp {tag: "ValuesExp"; params: CExp[];}
export const makeValuesExp = (params: CExp[]): ValuesExp =>
    ({tag: "ValuesExp", params: params});
export const isValuesExp = (x: any): x is ValuesExp => x.tag === "ValuesExp";

// let-values expression contains special bingings because it contains tuple as vars and vals
export interface LetValuesExp {tag: "LetValuesExp";  bindings: LetValuesBinding[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: LetValuesBinding[], body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", bindings: bindings, body: body});
export const isLetValuesExp = (x: any): x is LetValuesExp => x.tag === "LetValuesExp";

// var are VarDecl[] because it is a tuple 
export interface LetValuesBinding {tag: "LetValuesBinding"; var: VarDecl[]; val: CExp; }
export const makeLetValuesBinding = (v: VarDecl[], val: CExp): LetValuesBinding =>
    ({tag: "LetValuesBinding", var: v, val: val});
export const isLetValuesBinding = (x: any): x is LetValuesBinding => x.tag === "LetValuesBinding";
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    ...
    ! isGoodLetValuesBindings(bindings) ? makeFailure(`Invalid let-values-bindings: ${JSON.stringify(bindings)}`) :
    safe2((bdgs: LetValuesBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
    (parseLetValuesBindings(bindings), mapResult(parseL5CExp, body));

const parseLetValuesBindings = (bindings: [Sexp, Sexp][]): Result<LetValuesBinding[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeLetValuesBinding, vds, vals)))
    (mapResult(parseTupleToVarDecls, map((b) => b[0], bindings)), mapResult(parseL5CExp, map(b => b[1], bindings)));

// parse (x1 x2 x3 ...xn) to => [x1,x2,x3,x4..xn]
const parseTupleToVarDecls = (vars: Sexp): Result<VarDecl[]> =>
    isArray(vars) ? mapResult(parseVarDecl, vars):
    makeFailure("illegal sexp");


const isGoodLetValuesBindings = (bindings: Sexp): bindings is [Sexp, Sexp][] =>
    isArray(bindings) && allT(isArray, bindings) && allT(isArray, map((x) => x[0], bindings));
...
const unparseValuesExp = (ve: ValuesExp): Result<string> =>
    isEmpty(ve.params) ? makeOk(`(values)`):
    bind(mapResult(unparse, ve.params), (strs:String[]) =>
    makeOk(`(values ${strs.join(' ')})`));


const unparseLetValuesExp = (ve: LetValuesExp): Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
    (unparseLetValuesBindings(ve.bindings), unparseLExps(ve.body));


const unparseLetValuesBindings = (bindings: LetValuesBinding[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`((${vd}) ${val})`))
    (bind(mapResult(unparseVarDecl, bdg.var), (x: String[])=> makeOk(x.join(" "))), unparse(bdg.val)), bindings),
