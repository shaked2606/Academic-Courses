...
readlink(char* pathname, char* buf, uint bufsize){
  ...
  if(strlen(pathname) > bufsize){
    cprintf("pathname length bigger that buf\n");
    return -1;
  }

  if((ip = nameiparent(pathname, name)) == 0){
    cprintf("nameiparent failed\n");
    return -1;
  }

  ilock(ip);
  if(ip->type != T_DIR){
    cprintf("ip->type is not T_DIR\n");
    iunlockput(ip);
    return -1;
  }
  next = dirlookup(ip, name, 0);

  safestrcpy(buf, (char*) next->addrs, bufsize);
...
sys_symlink(void){
    ...
    safestrcpy((char*)ip->addrs, old, 128);
    iunlock(ip);
