...
public abstract class MicroService implements Runnable {
    ...
    private MessageBusImpl mesgBus;
    private ConcurrentHashMap<Class<? extends Message>, Callback> mesgsCallbacks;
    ...
    public MicroService(String name) {
        this.name = name;
        this.mesgBus = MessageBusImpl.getInstance();
        this.mesgsCallbacks = new ConcurrentHashMap<>();
    }
	...
	protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
    	mesgBus.subscribeEvent(type, this);
    	mesgsCallbacks.put(type, callback);
    	}
	...
	protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	mesgBus.subscribeBroadcast(type, this);
    	mesgsCallbacks.put(type, callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        Future<T> future = mesgBus.sendEvent(e);
        return future;
    }
    ...
    public final void run() {
    	...
    	mesgBus.register(this);

    	initialize();
    	
    		while (!terminated) {
    			try {            
    				Message currMessage = mesgBus.awaitMessage(this);
    				Callback<Message> callback = mesgsCallbacks.get(currMessage.getClass());

    				if (currMessage instanceof Message) {
    					callback.call(currMessage);
    				}
