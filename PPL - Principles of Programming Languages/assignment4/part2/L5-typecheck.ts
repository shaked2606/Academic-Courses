// L5-typecheck
// ========================================================
import { equals, zipWith, flatten, chain, map } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, VarDecl, isVarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, makeEmptyTupleTExp, isNonEmptyTupleTExp, isEmptyTupleTExp } from "./TExp";
...
export const typeofValues = (exp: ValuesExp, tenv: TEnv): Result<TExp> =>
    isEmpty(exp.params) ? makeOk(makeEmptyTupleTExp()) :
    bind(mapResult(param => typeofExp(param, tenv), exp.params), (texps:TExp[]) => makeOk(makeNonEmptyTupleTExp(texps)));


export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const vars = map((b)=> b.var,exp.bindings); // VarDecl[][]
    const flat_vars = flatten(vars); // VarDecl[]
    const string_vars = map((b)=>b.var, flat_vars); // string[]
    const texp_vars = map((b) => b.texp, flat_vars);
    const varTEs = map((vs) => map((v) => v.texp, vs), vars);

    const vals = map((b) => b.val, exp.bindings);

    const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv), (typeOfVal: TExp) => 
    isEmptyTupleTExp(typeOfVal) ? makeFailure("empty val tuple is illegal ") :
    isNonEmptyTupleTExp(typeOfVal) ? zipWithResult((minivarTE, minival) => checkEqualType(minivarTE, minival, exp), varTE, typeOfVal.TEs) :
    makeFailure("value is not a tuple")), varTEs, vals);

    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(string_vars, texp_vars, tenv)));
}
