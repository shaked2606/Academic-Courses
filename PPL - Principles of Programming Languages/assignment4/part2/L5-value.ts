...
export const tupleToString = (tuple: Tuple): string =>
    isEmpty(tuple.vals) ?  ``: // if no values - return empty string
    `(${join(' ', tuple.vals)})`
...
export const valueToString = (val: Value): string =>
    ...
    isPrimOp(val) ? val.op :
    isTuple(val) ? tupleToString(val) : // converting tuple to string
