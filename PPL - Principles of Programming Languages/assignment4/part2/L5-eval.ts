...
import { map, repeat, zipWith, flatten, chain } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, LetValuesBinding, makeBinding } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, SExpValue, isTuple, makeTuple } from "./L5-value";
import { isEmpty, first, rest, allT } from '../shared/list';
import { Result, makeOk, makeFailure, mapResult, safe2, zipWithResult, bind } from "../shared/result";
...
const evalValues = (exp: ValuesExp, env: Env): Result<Value> =>
    isEmpty(exp.params) ? makeOk(makeTuple([])):
    bind(mapResult(param => applicativeEval(param, env), exp.params),
    (vals: Value[]) => makeOk(makeTuple(vals)));


// bindings: [[x1, x2, ..xn], [y1, y2, ...yn]... ] body: [CExp1, CExp2,...CExpn] 
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    const cvals = mapResult((v : CExp) => applicativeEval(v, env), map((b : LetValuesBinding) => b.val, exp.bindings)); // tuples of vals SExpValue[]
    const vars = map((b: LetValuesBinding) => b.var, exp.bindings); // array of arrays of vars VarDecl[][]
    const flat_vars = flatten(vars);
    const string_vars = map((b: VarDecl) => b.var, flat_vars);

    const bindings = bind(cvals, (cvals:SExpValue[]) =>  
    zipWithResult((v:VarDecl[], cval:SExpValue) =>  
                isTuple(cval) && (cval.vals.length === v.length) ? makeOk(zipWith((v:VarDecl, val:SExpValue): [VarDecl, Value] => [v, val], v, cval.vals)) :
                makeFailure<[VarDecl, SExpValue][]>("invalid binding"), vars, cvals));
    
    const flat_bindings = bind(bindings, bindings=> makeOk(chain(b=>b,bindings)));

    return bind(flat_bindings, flat_bindings => evalSequence(exp.body, makeExtEnv(string_vars, flatten(map((b) => b[1], flat_bindings)), env)));
